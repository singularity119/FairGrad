...
method=fairgrad
alpha=2.0
init_type=rbd_multitask_curvature_spectral_once # 设置为空字符串表示跳过初始化
steps=0
...
if [ -z "$init_type" ]; then
    ...
    nohup python -u trainer_init.py \
        ...
        > "$LOG_DIR/$method-alpha$alpha-no-init-sd$seed-steps$steps-$timestamp.log" 2>&1 &
else
    # 有初始化类型
    seed=1
    mkdir -p "$LOG_DIR/fairgrad"
    nohup python -u trainer_init.py \
        --method=$method --seed=$seed --init_type=$init_type --alpha=$alpha --steps=$steps \
        --gpu 0 --data-path /root/autodl-tmp/dataset/celeba --save-dir "$SAVE_DIR" \
        > "$LOG_DIR/fairgrad/$method-alpha$alpha-$init_type-sd$seed-steps$steps-$timestamp.log" 2>&1 &
fi
